 ...
 */
const filesReg = (root: string, reg: { test: (arg0: string) => unknown; }, isPath = false) => {
  // 获取根目录下的文件夹名称
 ...
 */
const configFilesReg = (configReg: RegExp, reg: RegExp, getNewConfigFile: { (readConfigFile: any, template: any): any; (readConfigFile: any, template: any): any; (arg0: string, arg1: string): any; }) => {
...
const preview = async () => {
  ...
  try {
    ...
    // vite 模版
    const generateViteConfig = (readConfigFile: string, template: any) => readConfigFile.replace('defineConfig({', `defineConfig({\n base: '/${template}',`);
    configFilesReg(/^template-vite/, /^vite.config.*/, generateViteConfig);

    // farm 模版
    const generateFarmConfig = (readConfigFile: string, template: any) =>
